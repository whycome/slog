...
$uri: 请求的文件和路径,不包含'?'和'#'之类的东西
$request_uri: 请求的整个字符串,包含后边的东西
...
rewrite: 只能放在server{}, location{}, if{}中,只对域名后边除去参数外的字符串起作用;
...
$host: 请求中的host字段, 如果请求中的host字段不可用,则为服务器处理请求的服务器名,
...
http://106.75.97.4:5055/debug/kugou_album_api
