#第一步
#由appid 微信回调url 最初的请求request.path 获取code;
#第二步
#由appid appsecrect code 获取openid和access_token;
#第三步:
#由open_id access_token 获取用户信息
    ...
    def wrap(*args, **kw):
        ...
        if not openid:
            url = WX_GET_USERINFO_CODE.format(   #需要一个appid，回调处理code的url，请求的state
                    appid = yourappid,
                    redirect_uri = quote(WxCallBackUrl)
